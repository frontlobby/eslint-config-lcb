...
export class ImportAlignment extends BaseESLintRule {
	...
	hasExplicitCollapseExtraSpaces: boolean;
	options: ResolvedAlignImportsOptions;
	...
	constructor(context: Rule.RuleContext) {
		...
		this.options                        = Object.assign({
			...
			maxSpaces           : 25,
			maxLenBuffer        : 5,
		}, context.options[0] as AlignImportsOptions | undefined);
	}
	...
	getFromKeyword(node: ImportDeclaration): AST.Token {
	...
	reportUnalignedImportStatement(node: ImportDeclaration, diff: number): void {
		this.context.report({
			node,
	...
	reportUnalignedImportGroup(lineDiffs: LineDiff[]): void {
	...
	getImportSpacingFix(node: ImportDeclaration, diff: number, fixer: Rule.RuleFixer): Rule.Fix {
		const sourceCode    = this.sourceCode;
	...
	isSuitableImport(node: ImportDeclaration): boolean {
		return node.type === 'ImportDeclaration' && node.specifiers.length >= 1 && isSingleLine(node);
	}

	findSurroundingImports(node: ImportDeclarationNode): ImportDeclarationNode[] {
		const self = this;
		...
		function findImport(importNode: ImportDeclarationNode, direction: -1 | 1): ImportDeclarationNode[] {
	...
	getLineInfo(node: ImportDeclaration): ImportLineInfo {
		const sourceCode = this.sourceCode;
	...
	shouldReportGroupedDiff(lineDiffs: LineDiff[], shouldCollapse: boolean): boolean {
	...
	getAlignmentInfo(currentLine: ImportLineInfo, lines: ImportLineInfo[]): AlignmentInfo {
	...
	applyMinColumnWidth(alignmentColumn: number): number {
		return this.options.minColumnWidth ? Math.max(alignmentColumn, this.options.minColumnWidth) : alignmentColumn;
	}

	shouldCollapseExtraSpaces(lines: ImportLineInfo[]): boolean {
	...
	resolveCollapsedGroup(currentLine: ImportLineInfo, lines: ImportLineInfo[], strategy: 'widest' | 'shortest'): AlignmentInfo {
...
interface AlignImportsOptions {
	collapseExtraSpaces?: boolean;
	minColumnWidth?: number;
	/** Wrap single-line named imports when the line exceeds this length. */
	maxLen?: number;
	/**
	 * Characters subtracted from `maxLen` when deciding whether to unwrap a
	 * multiline import, and added when deciding whether to wrap. Prevents
	 * wrap/unwrap oscillation near the limit. Default: `5`. Use `0` to disable.
	 */
	maxLenBuffer?: number;
	maxSpaces?: number;
}

interface ResolvedAlignImportsOptions {
	collapseExtraSpaces: boolean;
	minColumnWidth: number;
	maxSpaces: number;
	maxLen?: number;
...
interface ImportParent {
	body: ImportDeclarationNode[];
}

interface ImportDeclarationNode extends ImportDeclaration {
	parent: ImportParent;
	loc: NonNullable<ImportDeclaration['loc']>;
}

interface ImportLineInfo {
...
interface AlignmentInfo {
	...
	groupLines: ImportLineInfo[];
}

function getImportPrefix(sourceCode: SourceCode, node: ImportDeclaration): string | null {
...
function getNamedSpecifierTexts(sourceCode: SourceCode, node: ImportDeclaration): string[] {
...
function buildSingleLineImportText(sourceCode: SourceCode, node: ImportDeclaration): string {
...
function getCollapsedLineLength(sourceCode: SourceCode, node: ImportDeclaration): number {
...
function hasImportSemicolon(sourceCode: SourceCode, node: ImportDeclaration): boolean {
...
function collapseImportLayout(text: string): string {
	return text
		.replace(/;\s*$/, '')
		.split('\n')
		.map(line => line.trim())
...
function hasCanonicalMultilineImportIndent(sourceCode: SourceCode, node: ImportDeclaration, current: string): boolean {
	if (isSingleLine(node)) {
		return true;
	}
...
function packSpecifierLines(specifiers: string[], maxLen: number, baseIndent: string): string[] {
	const continuationIndent = getChildIndent(baseIndent);
	const lines              = [];
	let lineStart            = continuationIndent;
	let bucket: string[]     = [];

	function renderLine(start: string, specs: string[], trailingComma: boolean): string {
		if (specs.length === 0) {
	...
	function remainingFitsOnLine(start: string, fromIndex: number): boolean {
...
function buildWrappedImportText(sourceCode: SourceCode, node: ImportDeclaration, maxLen: number): string {
	const baseIndent     = getLineIndent(node, sourceCode);
	const prefix         = getImportPrefix(sourceCode, node);
	const specifiers     = getNamedSpecifierTexts(sourceCode, node);
