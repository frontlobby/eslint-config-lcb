...
export class EnumValueAlignment extends BaseESLintRule {
    ...
    checkEnum(node) {
        const { members } = node;
        const lines = members.map(member => this.getLineInfo(member)).filter((line) => Boolean(line));
        if (_.isEmpty(lines)) {
            return;
        }
        ...
        for (const block of blocks) {
            const blockLines = block.map(member => this.getLineInfo(member)).filter((line) => Boolean(line));
